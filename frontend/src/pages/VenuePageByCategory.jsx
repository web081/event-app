// VenuePageByCategory.js
import React, { useState, useEffect } from "react";
import { useParams, Link } from "react-router-dom";
import { MapPin, Star, ChevronLeft, ChevronRight } from "lucide-react";
import backendURL from "../config";

const VenuePageByCategory = () => {
  ...
  useEffect(() => {
    const fetchCategoryVenues = async () => {
      try {
        ...
        const categoryVenues = venueData.filter(
          (venue) => venue.type?.toLowerCase() === categoryId?.toLowerCase()
        );
  ...
  const formatCapacity = (capacity) => {
    if (!capacity) return "500+ Guests";
    const [min, max] = capacity.split("-");
    return max ? `${min}-${max} Guests` : `${min}+ Guests`;
  };
    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {currentVenues.map((venue) => (
          <div
            key={venue._id}
            className="group h-full rounded-lg overflow-hidden shadow-sm transition-all duration-300 hover:shadow-lg"
          >
...
export default VenuePageByCategory;
