...
const AdminDashboard = () => {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const visitsResponse = await fetch(`${backendURL}/api/visits/statistics`);
  ...
  const fetchBusinessStats = useCallback(async () => {
    try {
      const [servicesRes, venuesRes, reviewsRes, blacklistRes] =
        await Promise.all([
          fetch(`${backendURL}/api/getAllServices`),
          fetch(`${backendURL}/api/getAllVenues`),
          fetch(`${backendURL}/api/getAllReviews`),
          fetch(`${backendURL}/api/getBlacklistedBusiness`),
        ]);

      const [servicesData, venuesData, reviewsData, blacklistData] =
        await Promise.all([
          servicesRes.json(),
          venuesRes.json(),
          reviewsRes.json(),
      ...
      setBusinessStats({
        services: {
          ...
          verified: servicesData.verified || 0,
          unverified: servicesData.unverified || 0,
          lastMonth: servicesData.lastMonth || 0,
          trend: servicesData.trend || 0,
        },
        venues: {
          total: venuesData.total || 0,
          verified: venuesData.verified || 0,
          unverified: venuesData.unverified || 0,
          lastMonth: venuesData.lastMonth || 0,
          trend: venuesData.trend || 0,
        },
        reviews: {
          total: reviewsData.total || 0,
          lastMonth: reviewsData.lastMonth || 0,
          trend: reviewsData.trend || 0,
        },
        blacklisted: {
          total: blacklistData.total || 0,
          lastMonth: blacklistData.lastMonth || 0,
          trend: blacklistData.trend || 0,
        },
      });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    fetchTotalRegUsers();
    fetchBusinessStats();
  }, [fetchStats, fetchTotalRegUsers, fetchBusinessStats]);
      ...
      {showVerification ? (
        <div className="space-y-2">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="text-white">{stats.unverified}</span>
          </div>
        </div>
      ) : (
        <div className="flex items-center text-sm">
          ...
          <span className="text-gray-400">{stats.lastMonth} Last month</span>
        </div>
      )}
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <BusinessStatCard
            ...
            stats={businessStats.services}
            icon={Building}
            showVerification
          />
          <BusinessStatCard
            title="Venue Listings"
            stats={businessStats.venues}
            icon={MapPin}
            showVerification
          />
          <BusinessStatCard
            title="Total Reviews"
            stats={businessStats.reviews}
            icon={Star}
          />
          <BusinessStatCard
            title="Blacklisted Business"
            stats={businessStats.blacklisted}
            icon={AlertTriangle}
          />
        </div>

        {/* Third row - Newsletter and Visitor Activity */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl p-6 shadow-lg">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-gray-400 text-sm font-medium uppercase">
